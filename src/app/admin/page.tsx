...
export default function AdminPage() {
  ...
  const orgColumns: ColumnDef<Org>[] = [
    ...
    },
    {
      ...
      header: "Tier",
      cell: (row) => TIER_LABELS[row.pricingTier] ?? row.pricingTier,
    },
    {
      ...
      cell: (row) => (
        <span
          className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${
            STATUS_COLORS[row.subscriptionStatus] ?? "bg-gray-100 text-gray-800"
          }`}
        >
    ...
    },
    {
      ...
      cell: (row) => (
        <span
          className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${
            STATUS_COLORS[row.onboardingStatus] ?? "bg-gray-100 text-gray-800"
          }`}
        >
